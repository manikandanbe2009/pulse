...
def insurance_analysis():
    ...
    st.write("This section provides an analysis of Insurance across different states and quarters.")
    ...
    if not df_transaction.empty:  
        ...
        fig_state.update_traces(
            hovertemplate=(
                "<b>State:</b> %{x}<br>"
                "<b>Insurance Count:</b> %{customdata[0]}<br>"
        ...
        st.markdown("### 🏆 Top 10 Districts by Insurance Amount")
        fig_district = px.bar(
            top10_districts,
            x="district",
            y="total_amount",
        ...
        fig_district.update_traces(
            hovertemplate="<b>Insurance Count:</b> %{customdata[0]}<br><b>Amount:</b> ₹%{customdata[1]}<br><b>District:</b> %{x}<extra></extra>",
          
        )
        st.plotly_chart(fig_district, use_container_width=True)

        st.markdown("### 🏆 Top 10 Pincodes by Insurance Amount")
        fig_pincode = px.bar(
            top10_pincodes,
            x="pincode",
            y="total_amount",
            color="pincode",
            barmode='group',
            title="Top 10 Pncodes by Insurance Amount",
        ...
        fig_pincode.update_traces(
            hovertemplate="<b>Insurance Count:</b> %{customdata[0]}<br><b>Amount:</b> ₹%{customdata[01]}<br><b>Pincode:</b> %{x}<extra></extra>",
        ...
        with tab1:
            ...
            with col1:
                st.metric("Total Insurance Count (Lakhs)", f"{top_states['total_count'].sum():,.2f}")
                fig_states = px.bar(
                    top_states,
                    x='state',
                    y='total_count',
                    color='total_count',
                    barmode='group',
                    title="Top 20 State Insurance  Count",
            ...
            with col2:
                st.metric("Total Insurance  Amount", f"₹ {map.format_inr_short(top_states['total_amount'].sum())}")
                fig_states2 = px.bar(
                    top_states,
        ...
        with tab2:
            st.write("Top 20 Districts based on Insurance")
            col1, col2 = st.columns(2)
            with col1:
                st.metric("Total Insurance Count (Lakhs)", f"{top_districts['total_count'].sum():,.2f}")
                fig_states5 = px.bar(
                    top_districts,
                    x='district',
                    y='total_count',
                    color='district',
                    barmode='group',
                    title="Top 20 District Insurance Count",
            ...
            with col2:
                st.metric("Total Insurance Amount", f"₹ {map.format_inr_short(top_districts['total_amount'].sum())}")
                fig_states2 = px.bar(
                    top_districts,
                    x='total_amount_foramtted',
                    y='district',
            ...
            col3, col4 = st.columns(2)
            with col3:
                st.metric("Total District Insurances Count (Lakhs)", f"{top_districts['total_count'].sum():,.2f}")
                fig_scatter1 = px.scatter(
                    top_districts,
                    x='district',
                    y='total_count',
                    color='total_count',
                    title="Top 20 State District Insurances Count",
                    labels={"total_count": "Total", "district": "District"},
                    height=600
                )
                ...
                st.plotly_chart(fig_scatter1, use_container_width=True)
            with col4:
                st.metric("Total Insurance Insurances Amount", f"₹ {map.format_inr_short(top_districts['total_amount'].sum())}")
                fig_scatter2 = px.scatter(
                    top_districts,
                    x='total_amount_foramtted',
                    y='district',
        ...
        with tab3:
            st.write("Top 20 Pincode based on Insurance ")
            col1, col2 = st.columns(2)
            with col1:
                st.metric("Total Insurance Count (Lakhs)", f"{top_pincode['total_count'].sum():,.2f}")
                fig_states5 = px.bar(
                    top_pincode,
                    x='pincode',
                    y='total_count',
                    color='pincode',
                    barmode='group',
                    title="Top 20 Pincode Insurance Count",
                ...
                st.plotly_chart(fig_states5, use_container_width=True)
            with col2:
                st.metric("Total Insurance Amount", f"₹ {map.format_inr_short(top_pincode['total_amount'].sum())}")
                fig_states2 = px.bar(
                    top_pincode,
                    x='total_amount_foramtted',
                    y='pincode',
            ...
            col3, col4 = st.columns(2)
            with col3:
                st.metric("Total Pincode Insurances Count (Lakhs)", f"{top_pincode['total_count'].sum():,.2f}")
                fig_scatter1 = px.scatter(
                    top_pincode,
                    x='pincode',
                    y='total_count',
                    color='total_count',
                    title="Top 20 Pincode Insurances Count",
                    labels={"total_count": "Total", "pincode": "Pincode"},
                    height=600
                )
        ...
        with tab4:
            ...
            col1, col2 = st.columns(2)
            with col1:
                st.metric("Total Insurance Count (Lakhs)", f"{top_year_quarter['total_count'].sum():,.2f}")
                fig_tab4_count = px.bar(
                    top_year_quarter,
                    x="year_quarter",
                    y="total_count",
                    color="year_quarter",
                    title="Top 20 Year Quarter by Insurance Count",
            ...
            with col2:
                st.metric("Total Insurance Amount", f"₹ {map.format_inr_short(district_df['total_amount'].sum())}")
                fig_tab4_2 = px.bar(
                    top_year_quarter,
                    x="year_quarter",
                    y="total_amount",
                    color="year_quarter",
                    title="Top 20 Year Quarter by  Insurance Amount",
            ...
            with col3:                   
                ...
                fig_trend = px.line(
                    ...
                    y="total_amount",
                    markers=True,
                    title="Top 20 Quarterly Trend of Insurance Transaction Amount",
                    labels={"x": "Year-Quarter", "total_amount": "Transaction Amount (₹)"}
                )
            ...
            with col4:
                ...
                fig_trend = px.line(
                    ...
                    y="total_count",
                    markers=True,
                    title="Top 20 Quarterly Trend of Insurance Transaction Count",
                    labels={"x": "Year-Quarter", "total_count": "Transaction Count"}
                )
                st.plotly_chart(fig_trend, use_container_width=True)
        ...
        with tab5:
            ...
            with tab1:
                col1, col2 = st.columns(2)
                with col1:
                    st.metric("Total Insurance Count (Lakhs)", f"{top_states['total_count_lakh'].sum():,.2f}")
                    fig_states = px.bar(
                        top_states,
                        x="state",
                        y="total_count_lakh",
                        ...
                        labels={"total_count_lakh": "Insurances (Lakhs)", "state": "State"},
                        height=500
                    )
            ...
            with tab2:
                col1, col2 = st.columns(2)
                with col1:
                    st.metric("Total Insurance Count (Lakhs)", f"{top_districts['total_count_lakh'].sum():,.2f}")
                    fig_districts = px.bar(
                        ...
                        labels={"total_count_lakh": "Insurances (Lakhs)", "district": "District"},
                        height=600
                    )
            ...
            with tab3:
                col1, col2 = st.columns(2)
                with col1:
                    st.metric("Total Insurance Count (Lakhs)", f"{top_pincodes['total_count_lakh'].sum():,.2f}")
                    fig_pincodes = px.bar(
                        top_pincodes,
                        x="pincode",
                        y="total_count_lakh",
                        ...
                        labels={"total_count_lakh": "Insurance (Lakhs)", "pincode": "Pincode"},
                        height=600
                    )
                ...
                with col2:
                    st.metric("Total Insurance Amount", f"₹ {map.format_inr_short(top_pincodes['total_amount'].sum())}")
